...
import {COLLECTIONS} from '../services/games.service';
import {ScreenDirective} from '../shared/screen.directive';
import {Base, BaseResponse, GameMetadataResponse, GameResponse, NameResponse, NoteResponse, TagResponse, Time, User} from '../shared/types';
                ...
                this.firestore.doc(collection + '/' + doc.id)
                    ...
                    .subscribe(names => {
                      for (const name of names) {
                        this.firestore
                            .doc(
                                collection + '/' + doc.id + '/names/' + name.id)
                            .delete();
                        namecount++;
                      }
                      namedonecount++;
    ...
    this.firestore.collection<User>(COLLECTIONS.USERS)
        ...
        .pipe(
            ...
            switchMap(tags => {
              ...
              return this.firestore.collection<GameResponse>(COLLECTIONS.GAMES)
                  .valueChanges({idField: 'id'})
                  .pipe(take(1));
            }),
            switchMap(games => {
                    ...
                    .subscribe(names => {
                      for (const name of names) {
                        if (name.name && !name.mongoID) {
                          ...
                          // console.log('and name for ', game.name, newName);
                          newNameCollection.doc(id).set(newName);
                          nameCount++;
                        }
                      }
                      namesDoneCount++;
              ...
              return this.firestore.collection<NoteResponse>(COLLECTIONS.NOTES)
                  .valueChanges({idField: 'id'})
                  .pipe(take(1));
            }),
            map(notes => {
